...
(function () {
  ...
  function nextLecture() {
    ...
    // Sprint 2 Exp2: Room/Box 허브가 없는 선형 Deck이므로 CURRICULUM.boxes 대신
    // window.DECK_SECTIONS(평탄화된 Section 배열, deck-main.js)를 순회한다.
    const all = window.DECK_SECTIONS || [];
  ...
  // Sprint 2 Exp3 Variant A: Fan(Section 허브)과 deck(슬라이드) 모두 몰입 구간
  const IMMERSIVE_ROUTES = { fan: true, deck: true };
  ...
  function openLecture(lecture, box, slideIndex) {
    ...
    nameEl.textContent = `${String(lecture.id).padStart(2, "0")} · ${lecture.title}`;
    // Sprint 2 Exp2: box는 더 이상 DOM 요소가 아니라 accent 인덱스(숫자)를 직접 받는다
    // (Room이 없으므로 CURRICULUM.boxes.indexOf(box) 조회가 성립하지 않음 — §07 기술 의존성 참고)
    if (box != null) document.documentElement.dataset.slideBox = String(box);
  ...
  function show(i) {
    ...
    if (activeLecture) App.Router.replace("deck/" + (window.DECK_SECTIONS || []).indexOf(activeLecture) + "/" + i);
    revealChrome();   // 슬라이드 바뀔 때 네비를 잠깐 보여줬다 숨김
  }
  const next = () => {
    if (current === slides.length - 1) {          // 마지막 슬라이드 → 다음 강의로
      const nl = nextLecture();
      if (nl) App.Router.go("deck/" + (window.DECK_SECTIONS || []).indexOf(nl));
      return;
    }
    show(current + 1);
  };
  ...
  // Sprint 2 Exp3 Variant A: Section을 마치고 나가면 Fan(Section 선택 허브)으로 돌아간다
