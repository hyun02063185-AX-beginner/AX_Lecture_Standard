...
(function () {
  ...
  /* ---------- 히든코드 치트 플래그 (order/히든코드_지시서.md) ----------
     ...
     기존 변수(VARIANT_PERSONAL·isSkinUnlocked 등)에 "치트도 확인" 한 줄만 얹는다.
  ...
  /* ---------- 콘텐츠 variant — 경험판 스위치 ----------
     ...
     해시 라우팅은 search를 건드리지 않으므로 세션 동안 자연 유지. URL 또는 무대뒤.경험판 치트
     플래그(재입력 시 해제, 페이지 새로고침으로 반영)로 켜진다. */
  const VARIANT_PERSONAL = /[?&]variant=personal(?:&|$)/.test(location.search) || cheatFlag(CHEAT_VARIANT_KEY);
  ...
  /* ---------- 스킨 해금 Lv (site-config 주도) ----------
     ...
     Lv 무관 전부 해금, 무대뒤.스킨 치트 플래그도 전부 해금(즉시 반영 — 새로고침 불요). */
  ...
  function updateStateButton(c) {
    // Sprint 2 Exp1: 게이미피케이션(업그레이드/통달 상태) 제거 — 순수 진행도 초기화 버튼만 유지
  ...
  function renderCelebrate() {
    ...
    if (mastered) {
      if (titleEl) titleEl.textContent = SITE_CONFIG.completionTitleMastered || "깊은 깨달음에 도달하셨습니다.";
      if (subEl) subEl.textContent = "Lv" + MAX_LEVEL + " · " + RANKS[MAX_LEVEL] + " · 통달 — 최고 경지에 이르렀습니다";
      if (upBtn) upBtn.hidden = true;
    } else {
      const next = Level.n + 1;
      if (titleEl) titleEl.textContent = SITE_CONFIG.completionTitle || "훌륭히 완주하셨습니다.";
      if (subEl) subEl.textContent = TOTAL + "강 완주 · 업그레이드하면 Lv" + next + " · " + RANKS[next] + (next >= MAX_LEVEL ? " (통달)" : "");
  ...
  const App = window.App = {
    ...
    // 히든코드 전용 — 코드 입력칸 파서(telemetry.js)가 호출, 즉시 반영(새로고침 불요)
    cheatSkinUnlock(on) {
